...
if (!function_exists('get_table_last_order')) {
	...
	{
		return htmlentities(get_staff_meta(get_staff_user_id1(), $tableID . '-table-last-order'));
	}
}
...
if (!function_exists('data_tables_init1')) {
	...
	$rResult = $Manufacturing_model->manufacturing_run_query($sQuery);
	...
	SELECT FOUND_ROWS()
	';
	$_query         = $Manufacturing_model->manufacturing_run_query($sQuery);
