<script>
  ...
  (function($) {
    ...
    var _table_api = initDataTable(table_commodity_list, "<?php echo get_uri("manufacturing/table_commodity_list") ?>", [0], [0], ProposalServerParams,  [1, 'desc']);
  ...
  $("body").on('click', '.tagit-close', function() {
    ...
    if(tag_id){
      ...
      $.post("<?php echo get_uri("warehouse/delete_item_tags/") ?>"+tag_id).done(function(response) {
        response = JSON.parse(response);
  ...
  function show_detail_item(el){
    ...
    var _table_api = initDataTable(table_out_of_stock,  "<?php echo get_uri("warehouse/table_out_of_stock") ?>", true, '', ProposalServerParams1);
    
    var table_expired = $('table.table-table_expired');

    var _table_api = initDataTable(table_expired, "<?php echo get_uri("warehouse/table_expired") ?>", true, '', ProposalServerParams1);
  ...
  "use strict";
    ...
    if(mass_delete == true){
      ...
      setTimeout(function() {
        ...
        $.post("<?php echo get_uri("manufacturing/warehouse_delete_bulk_action") ?>", data).done(function() {
          window.location.reload();
        }).fail(function(data) {
