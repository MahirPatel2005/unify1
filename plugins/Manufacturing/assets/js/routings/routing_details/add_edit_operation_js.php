<script>
	...
	var addMoreAttachmentsInputKey = 1;
		...
		$("body").on('click', '.add_more_attachments_file', function() {
			...
			newattachment.find('button svg').removeClass('feather-plus-circle').addClass('feather-x-');
			newattachment.find('button').find('svg').empty('').html('<line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line>');
		...
		function delete_operation_attachment(wrapper, id) {
			...
			$.get("<?php echo get_uri("manufacturing/delete_operation_attachment_file/") ?>" + id, function (response) {
		...
		function view_file_file(id, rel_id) {   
			...
			$("#contract_file_data").load("<?php echo get_uri("manufacturing/mrp_view_attachment_file/") ?>" + id + '/' + rel_id+'/'+'operation', function(response, status, xhr) {
				if (status == "error") {
