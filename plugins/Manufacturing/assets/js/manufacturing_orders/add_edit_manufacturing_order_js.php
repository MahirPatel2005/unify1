<script>
	...
	(function($) {
		...
		$(".select2").select2();
		setDatePicker("#date_deadline,#date_plan_from");
...
$('select[name="product_id"]').on('change', function() {
	...
	$.get("<?php echo get_uri("manufacturing/get_data_create_manufacturing_order/") ?>" + product_id, function (response) {

		$("select[name='bom_id']").html('');

		$("select[name='bom_id']").select2('destroy');
		$("select[name='bom_id']").select2();
...
$('select[name="bom_id"]').on('change', function() {
	...
	$.get("<?php echo get_uri("manufacturing/get_bill_of_material_detail/") ?>" + bill_of_material_id+'/'+product_id+'/'+product_qty, function (response) {
