...
$result = data_tables_init1($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix().'items.id', 'files'], '', [], $dataPost);
...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if ($aColumns[$i] == get_db_prefix().'item.title') {
			...
			if(mrp_has_permission('manufacturing_can_edit')) {
				$edit .=	'<li role="presentation"><a href="'.site_url('manufacturing/add_edit_product/product/'.$aRow['id']).'"  class="dropdown-item" data-toggle="sidebar-right" ><span data-feather="edit" class="icon-16"></span> ' . _l('edit') . '</a></li>';
			} 

			if(mrp_has_permission('manufacturing_can_delete')) {
				$delete .= '<li role="presentation">' .modal_anchor(get_uri("manufacturing/confirm_delete_modal_form"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), array("title" => app_lang('delete'). "?", "data-post-id" => $aRow['id'],"data-post-id2" => 'product', "data-post-function" => 'delete_product', "class" => 'dropdown-item' )). '</li>';
