	...
	<table class="header-style">
		<tr class="invoice-preview-header-row">
			...
			<td class="invoice-info-container width-35 vertical-align-top text-align-right">
				<span class="font-size-20 font-weight-bold" style  = "background-color: <?php echo html_entity_decode($color); ?>; color: <?php echo html_entity_decode($color_black) ?>;">&nbsp;<?php echo app_lang('wh_order_return'); ?>&nbsp;</span>

				<div class="line-height-5"></div>
				<span class="font-weight-bold" >&nbsp;<?php echo '#'.$order_return->order_return_number.' - '.$order_return->order_return_name; ?>&nbsp;</span>
				<div class="line-height-10"></div>
				<span><?php echo app_lang("datecreated") . ": " . format_to_date($order_return->datecreated, false); ?></span>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				...
				if(isset($client_info)){
					...
					<strong><?php echo html_entity_decode($order_return->company_name); ?> </strong>
					<div class="line-height-3"> </div><br />

					<b><?php echo app_lang("email") ?>: </b><?php echo html_entity_decode($order_return->email) ?><br />
					<b><?php echo app_lang("phonenumber") ?>: </b><?php echo html_entity_decode($order_return->phonenumber) ?>
				<?php } ?>
				<span></span>
			</td>
		</tr>
...
<table class="table-responsive width-100 color-black" >            
	...
	<?php
	$subtotal = 0;
	foreach ($order_return_details as $item_key => $item) {
	...
	if(isset($order_return)){
		$total_discount += (float)$order_return->discount_total  + (float)$order_return->additional_discount;
	}
	...
	<tr>
		...
		<td class="text-align-right width-20 border-1-solid-fff background-color-f4f4f4">
			<?php echo to_currency($order_return->subtotal); ?>
		</td>
	</tr>
	...
	<tr>
		...
		<td class="text-align-right width-20 border-1-solid-fff background-color-black">
			<span class="color-fff"><?php echo to_currency($order_return->total_after_discount); ?></span>
		</td>
	</tr>

</table>
<?php if (strlen($order_return->return_reason) > 0) { ?>
	<br />
	<div class="border-top-2-f2f2f2 color-black padding-0-0-20-0" ><br /><?php echo nl2br($order_return->return_reason); ?></div>
<?php }  ?>
<?php if (strlen($order_return->admin_note) > 0) { ?>

	<br />
	<div class="border-top-2-f2f2f2 color-black padding-0-0-20-0" ><br /><?php echo nl2br($order_return->admin_note); ?></div>
<?php }  ?>
<?php if (strlen($order_return->return_policies_information) > 0) { ?>

	<br />
	<div class="border-top-2-f2f2f2 color-black padding-0-0-20-0" ><br /><?php echo nl2br($order_return->return_policies_information); ?></div>
<?php }  ?>
