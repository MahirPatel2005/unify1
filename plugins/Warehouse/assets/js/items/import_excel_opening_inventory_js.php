<script>
	...
	(function($) {
		...
		{
			$( "#dowload_file_sample" ).append( '<a href="<?php echo html_entity_decode($site_url) ?>/plugins/Warehouse/Uploads/file_sample/Sample_import_opening_stock_vi.xlsx" class="btn btn-primary" ><?php echo _l('download_sample') ?></a>' );

		}else{
			$( "#dowload_file_sample" ).append( '<a href="<?php echo html_entity_decode($site_url) ?>/plugins/Warehouse/Uploads/file_sample/Sample_import_opening_stock_en.xlsx" class="btn btn-primary" ><?php echo _l('download_sample') ?></a>' );
		}
	...
	function uploadfilecsv(event){
		...
		$.ajax({ 
			url: "<?php  echo get_uri('warehouse/import_file_xlsx_opening_stock') ?>", 
			method: 'post', 
			data: formData, 
			contentType: false, 
			processData: false
