<script>
	...
	$(document).ready(function () {
		...
		<?php if(isset($unit_type_data)){ ?>
			var dataObject_pu = <?php echo json_encode($unit_type_data) ; ?>;
		<?php }else{?>
			var dataObject_pu = [];
		<?php } ?>
		...
		setTimeout(function(){
				...
				unit_type = new Handsontable(hotElement1, {
					contextMenu: true,
					manualRowMove: true,
					...
					colHeaders: [
					'<?php echo app_lang("unit_type_id") ?>',
					'<?php echo app_lang("unit_code") ?>',
					'<?php echo app_lang("unit_name") ?>',
					'<?php echo app_lang("unit_symbol") ?>',
	...
	$('.submit_commodity_modal').on('click', function() {
		...
		if(valid_edit_multiple_transaction){
			...
			$('input[name="add_handsontable_hs"]').val(JSON.stringify(unit_type.getData()));   
			$('#unit_type-form').submit(); 
		}
	});
</script>
