<script>
	...
	$(document).ready(function () {
		...
		<?php if(isset($style_type_data)){ ?>
			var dataObject_pu = <?php echo json_encode($style_type_data) ; ?>;
		<?php }else{?>
			var dataObject_pu = [];
		<?php } ?>
		...
		setTimeout(function(){
				...
				style_type = new Handsontable(hotElement1, {
					contextMenu: true,
					manualRowMove: true,
					...
					colHeaders: [
					'<?php echo app_lang("style_type_id") ?>',
					'<?php echo app_lang("style_code") ?>',
					'<?php echo app_lang("style_barcode") ?>',
					'<?php echo app_lang("style_name") ?>',
	...
	$('.submit_commodity_modal').on('click', function() {
		...
		if(valid_edit_multiple_transaction){
			...
			$('input[name="add_handsontable_hs"]').val(JSON.stringify(style_type.getData()));   
			$('#style_type-form').submit(); 
		}
	});
</script>
