<script>
	...
	$(document).ready(function () {
		...
		<?php if(isset($body_type_data)){ ?>
			var dataObject_pu = <?php echo json_encode($body_type_data) ; ?>;
		<?php }else{?>
			var dataObject_pu = [];
		<?php } ?>
		...
		setTimeout(function(){
				...
				body_type = new Handsontable(hotElement1, {
					contextMenu: true,
					manualRowMove: true,
					...
					colHeaders: [
					'<?php echo app_lang("body_type_id") ?>',
					'<?php echo app_lang("model_code") ?>',
					'<?php echo app_lang("model_name") ?>',
	...
	$('.submit_body_modal').on('click', function() {
		'use strict';
		...
		if(valid_edit_multiple_transaction){
			...
			$('.submit_body_modal').attr( "disabled", "disabled" );
			$('input[name="add_handsontable_hs"]').val(JSON.stringify(body_type.getData()));   
			$('#body_type-form').submit(); 
		}
	});
</script>
