<script>
	...
	/*required email,phonenumber,order_return_name,company_id*/
...
function wh_get_item_row_template(name, commodity_name, quantity, unit_name, unit_price, taxname, commodity_code, unit_id, tax_rate, discount, item_key)  {
	...
	var d = $.post("<?php echo get_uri("warehouse/get_order_return_row_template") ?>", {
		name: name,
...
function wh_calculate_total(){
	...
	$('.wh-additional_discount').html('<input type="number" class="form-control" name="additional_discount" min="0.0" step="any" value="' + additional_discount + '" readonly="true">');
...
$("body").on('change', '#company_id', function () {
	...
	if(company_id != null && company_id.length > 0){
		requestGetJSON("<?php echo get_uri("warehouse/wh_client_data/") ?>" + company_id +'/'+ receipt_delivery_type).done(function (response) {
...
$('select[name="rel_id"]').on('change', function() {
	...
	$.post("<?php echo get_uri("warehouse/order_return_get_item_data") ?>", data).done(function(response){
		response = JSON.parse(response);
...
function wh_sale_order_calculate_total(){
	...
	additional_discount = parseFloat(calculate_additional_discount(total_qty, total_quantity_default, additional_discount_default)).toFixed(decimal_places);
...
$("body").on('change', '#rel_type', function () {
	...
	$.get("<?php echo get_uri("warehouse/order_return_get_related_data") ?>", data).done(function(response){
		response = JSON.parse(response);
