<script>
	...
	(function($) {
		...
		<?php if(isset($send_mail_approve)){ 
			...
			data_send_mail.rel_id = <?php echo html_entity_decode($internal_delivery->id); ?>;
		...
		function send_request_approve(id){
			"use strict";
			var data = {};
			data.rel_id = <?php echo html_entity_decode($internal_delivery->id); ?>;
			data.rel_type = '4';

			data.addedfrom = <?php echo html_entity_decode($internal_delivery->addedfrom); ?>;
		...
		$("#print-invoice-btn").click(function () {
			...
			$.ajax({
				url: "<?php echo get_uri('warehouse/print_internal_delivery/' . $internal_delivery->id) ?>",
				dataType: 'json',
